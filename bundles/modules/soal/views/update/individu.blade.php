...
@section('content')
	...
	{{ Form::open() }}
		...
		{{ Form::btextarea('','name', $soal->name , array('class'=>'span12','rows'=>8)) }}

		@for($x = 1; $x<=5; $x++)
			<div class="row">
				<div class="span1">
					<?php if ($x == $soal->jawaban):?>
						<?php echo Form::radio('jawaban',$x, true);?>
					<?php else:?>
						<?php echo Form::radio('jawaban',$x);?>
					<?php endif;?>
				</div>
				<div class="span11">
					<?php echo Form::btextarea('','ops_'.$x, $soal->{"ops_$x"} ,array('class'=>'span12','rows'=>5)) ;?>
				</div>
			</div>
		@endfor
